...
export default function ClusterSelector() {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h2 className="text-xl font-semibold mb-4 text-white">Select your Kubernetes Cluster</h2>
        <p className="text-zinc-400 mb-6">Choose a cluster and configure it below.</p>
            ...
            <button
              ...
              className={`p-4 rounded-lg border-2 transition-all ${
                selectedCluster === cluster
                  ? "border-green-500 bg-zinc-800 text-green-500"
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-medium text-white">Select a configuration:</h3>
        ...
        <RadioGroup value={selectedConfig} onValueChange={setSelectedConfig} className="space-y-4">
          {configurations.map((config) => (
            <div
              key={config.id}
              className="flex items-center justify-between p-4 rounded-lg border border-zinc-800 hover:border-zinc-700"
            >
              <div className="flex items-center space-x-4">
                ...
                <div>
                  ...
                  <div className="text-sm text-zinc-400">{config.specs}</div>
                </div>
              </div>
              <div className="flex items-center space-x-4">
                <span className="text-zinc-400">{config.provider}</span>
                <span className="text-white">${config.price}/hr</span>
              </div>
            </div>
          ))}
        </RadioGroup>
      ...
      <div>
        ...
        <Input id="storage" type="number" placeholder="..." className="mt-2 bg-zinc-900 border-zinc-800 text-white" />
      </div>

      <Button className="bg-green-600 hover:bg-green-700">Save Cluster Configuration</Button>
    </div>
  )
}
